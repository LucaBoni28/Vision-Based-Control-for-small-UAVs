...
- [x] Finalize `README.md` for project handover to the supervisor and subsequent development teams.

### 📝 Daily Log
- Conducted a full, file-by-file code review of the OOP architecture.
- Authored extensive inline comments across all core classes (`config.py`, `flight_controller.py`, `mission_controller.py`, `distance_estimator.py`, etc.) to solidify understanding of the system's logic and data flow.
- Clarified complex edge-cases in documentation, specifically:
  - **Pitch Compensation**: Documented how the system dynamically polls the Pixhawk (`get_param("MNT1_STAB_TILT")`) to determine if a physical gimbal is active, automatically overriding software-based trigonometric pitch compensation.
  - **Coupled Velocity Limiter**: Documented the `r_stop` parameter, clarifying its role as a safety radius threshold that dynamically clamps forward velocity (`v_x`) to 0 if the target drifts too far off-center, prioritizing yaw-correction over approach speed.
  - **Calibration Workflow**: Validated the manual, non-blocking calibration state machine and removed deprecated parameters that were obsoleted by the manual start/stop workflow.
- **Handover Preparation**: Completely restructured the `README.md` to ensure high-level architecture, simulation/SITL capabilities, and operational commands (calibration, target locking, networking via Tailscale/MAVProxy) are clearly communicated for technical supervisors.
